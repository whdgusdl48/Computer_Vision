...
class DCGAN():
    ...
    def __init__(self,
        ...
        self.discriminator_optimizer = tf.keras.optimizers.Adam(0.00002,0.5)
    ...
    def generator_loss(self,fake_out):
        cross_entropy = tf.keras.losses.BinaryCrossentropy(from_logits=True)
        return tf.reduce_mean(cross_entropy(tf.ones_like(fake_out),fake_out))
    ...
    def build_generator(self):
        ...
        for i in range(len(self.generator_filters)):
            if i != len(self.generator_filters)-1:
                ...
                x = Conv2D(filters=self.generator_filters[i],
                                 kernel_size=(4,4),
                                 strides=self.generator_strides[i],
                                 padding='same',
                                 )(x)
